...
const _downloadAsMP4 = function (url, filename, artist, mp4FilePath) {
	console.log(`url: ${url}, filename: ${filename}, artist: ${artist}, mp4FilePath: ${mp4FilePath}`);
	return new Promise((resolve, reject) => {
		try {
			...
			const saveLocation = `${mp4FilePath}/${filename}.mp4`;
			video.pipe(fs.createWriteStream(saveLocation));

			video.on('end', function () {
				// const data = { artist: artist, title: filename };
